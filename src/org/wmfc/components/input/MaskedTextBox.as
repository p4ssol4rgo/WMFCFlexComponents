	...
	public class MaskedTextBox extends TextBox
	{
		...
		*/
		...
		override public function get text():String
		{
			...
			return _trimText ? StringUtils.trim(result) : result;
		}
		...
		*/
		...
		{
			var p:Number = this.selectionAnchorPosition;
			_position = p;
		}
		...
		private function interceptKey( event:flash.events.KeyboardEvent ) : void
		{
			if( event.keyCode == Keyboard.BACKSPACE ) {
				...
				if(StringUtils.trim(this.text).length == 0 || (this.selectionActivePosition - this.selectionAnchorPosition) == this.actualText.length) {
					_working = [];
					_position = 0;
					bMaskUpdated = true;
				}else{
					_position = selectionAnchorPosition;
					retreatPosition();
				...
				if((this.selectionActivePosition - this.selectionAnchorPosition) == this.actualText.length) {
					_working = [];
					_position = 0;
					bMaskUpdated = true;
				}
				...
				_position = this.selectionAnchorPosition;
				retreatPosition();
				event.preventDefault();
			}
		...
		{
			var start:Number = selectionAnchorPosition;
			
			if( event.relatedObject != null ) {
				start = 0;
				selectRange(selectionAnchorPosition, selectionActivePosition);
			} else {
...
/*			if((this.selectionActivePosition - this.selectionAnchorPosition) == this.actualText.length) {
		...
		protected function advancePosition(byArrow:Boolean=false) : void
		{
			...
			selectRange(_position, _position);
		}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if( bTextUpdated ) {
				...
				var t:Number = 0;
				var value:String = (_text == null ? "" : _text);
